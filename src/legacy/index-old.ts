...
export default {
  async scheduled(
    ...
    // GitHub Discovery daily report
    if (env.GITHUB_DISCOVERY_ENABLED === 'true' && env.OPENAI_API_KEY && 
        env.Z_API_INSTANCE_ID && env.Z_API_INSTANCE_TOKEN && env.Z_API_SECURITY_TOKEN && 
        env.GITHUB_DISCOVERY_WHATSAPP_NUMBER) {
      try {
        const { GitHubDiscovery } = await import('./modules/github-discovery');
        const githubDiscovery = new GitHubDiscovery({
          openAiApiKey: env.OPENAI_API_KEY,
          zApiInstanceId: env.Z_API_INSTANCE_ID,
          zApiInstanceToken: env.Z_API_INSTANCE_TOKEN,
          zApiSecurityToken: env.Z_API_SECURITY_TOKEN,
          whatsappNumber: env.GITHUB_DISCOVERY_WHATSAPP_NUMBER,
          enabled: true,
          twitterBearerToken: env.TWITTER_BEARER_TOKEN,
          twitterCredentials: env.TWITTER_USERNAME ? {
            username: env.TWITTER_USERNAME,
            password: env.TWITTER_PASSWORD,
            email: env.TWITTER_EMAIL,
          } : undefined,
          railwayApiUrl: env.GITHUB_SCRAPER_API_URL,
          railwayApiKey: env.GITHUB_SCRAPER_API_KEY,
        }, env.USER_CONFIGS);
        
        await githubDiscovery.runDailyDiscovery();
        console.log('GitHub discovery report sent successfully');
      } catch (error) {
        console.error('Error running GitHub discovery:', error);
      }
    } else {
      console.log('GitHub discovery disabled - missing environment variables or not enabled');
    }
  },
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === '/test-github-discovery' && request.method === 'POST') {
      return handleTestGitHubDiscovery(request, env);
    }
...
async function handleTestGitHubDiscovery(request: Request, env: Env): Promise<Response> {
  // Authentication
  const authHeader = request.headers.get('Authorization');
  if (authHeader !== `Bearer ${env.WEBHOOK_SECRET}`) {
    return new Response('Unauthorized', { status: 401 });
  ...
  if (!env.OPENAI_API_KEY || !env.Z_API_INSTANCE_ID || !env.Z_API_INSTANCE_TOKEN || 
      !env.Z_API_SECURITY_TOKEN || !env.GITHUB_DISCOVERY_WHATSAPP_NUMBER) {
    return new Response(JSON.stringify({ 
      success: false, 
      error: 'GitHub discovery not configured - missing environment variables' 
    }), {
      status: 400,
      headers: { 'Content-Type': 'application/json' }
    });
  ...
  try {
    ...
    const connectionTest = await githubDiscovery.testScraperConnection();
    
    // Run test discovery (doesn't send messages automatically)
    const result = await githubDiscovery.testDiscovery();
    
    // Send test message if projects found
    if (result.projects.length > 0) {
      await githubDiscovery.sendTestMessage(result.projects, result.totalPosts);
    }
    
    // Get storage stats
    const stats = await githubDiscovery.getStorageStats();
    
    return new Response(JSON.stringify({ 
      success: true, 
      message: 'GitHub discovery test completed',
      results: {
        scraperConnection: connectionTest,
        projectsFound: result.projects.length,
        totalPostsAnalyzed: result.totalPosts,
        ...
        storage: stats,
        enabled: githubDiscovery.isEnabled()
      }
    }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' }
    ...
    console.error('Error testing GitHub discovery:', error);
    return new Response(JSON.stringify({ 
      success: false, 
